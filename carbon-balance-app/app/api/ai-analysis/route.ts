...
export async function POST(request: Request) {
  try {
    const { municipality } = await request.json()

    console.log("[v0] Starting AI analysis for:", municipality.municipio)

    const prompt = `Eres un experto en análisis ambiental y balance de carbono. Analiza los siguientes datos del municipio de ${municipality.municipio}, Santander, Colombia:
